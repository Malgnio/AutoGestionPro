...
export default function InsuranceScreen() {
  ...
  async function loadInsurance() {
    ...
    const { data } = await supabase
      .from('insurance').select('*').eq('user_id', user.id)
  ...
  async function handleSave() {
    ...
    if (editingId) {
      const { error } = await supabase.from('insurance').update({
        customer_name: customerName, rut: formattedRut, chassis, insurance_type: insuranceType,
      }).eq('id', editingId)
      setSaving(false)
      if (error) { setError(error.message) } else { setShowForm(false); resetForm(); loadInsurance() }
    } else {
      ...
      const { error } = await supabase.from('insurance').insert({
        ...
        chassis, insurance_type: insuranceType, sale_month: saleMonth,
      })
      setSaving(false)
      if (error) { setError(error.message) } else { setShowForm(false); resetForm(); loadInsurance() }
    }
  ...
  const totalCommission = insurance.length * COMMISSION_PER_POLICY
      ...
      <View style={styles.main}>
        ...
        {!loading && (
          <View style={styles.kpiRow}>
            <View style={[styles.kpiCard, { backgroundColor: '#8E44AD' }]}>
              <Text style={styles.kpiLabel}>Seguros vendidos</Text>
              <Text style={styles.kpiValue}>{insurance.length}</Text>
              <Text style={styles.kpiSub}>del mes</Text>
            </View>
            <View style={styles.kpiCard}>
              ...
              <Text style={[styles.kpiSub, { color: Colors.textLight }]}>{insurance.length} × $23.000</Text>
            </View>
          </View>
        )}
          ...
          <ScrollView style={styles.tableContainer}>
            {insurance.length === 0 ? (
              <View style={styles.empty}>
                <Text style={styles.emptyText}>No hay seguros en {MONTHS[selectedMonth]} {selectedYear}</Text>
              </View>
            ) : (
                ...
                {insurance.map((item, index) => (
                  <View key={item.id} style={[styles.tableRow, index % 2 === 0 ? styles.rowEven : styles.rowOdd]}>
                    ...
                    <View style={[styles.cell, styles.cellType]}>
                      <View style={[styles.badge, { backgroundColor: TYPE_COLOR[item.insurance_type] }]}>
                        <Text style={styles.badgeText}>{item.insurance_type}</Text>
                      </View>
                    </View>
      ...
      <View style={[styles.drawer, showForm && styles.drawerOpen]}>
        ...
        <ScrollView style={styles.drawerBody} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.commissionNote}>
            <Text style={styles.commissionNoteText}>Comisión fija: $23.000 por seguro</Text>
          </View>
        </ScrollView>
